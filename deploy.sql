...
CREATE ADAPTIVE WAREHOUSE IF NOT EXISTS PGSYNC_WH
    WITH MAX_QUERY_PERFORMANCE_LEVEL = XSMALL
...
CREATE TABLE IF NOT EXISTS PGSYNC_DB.METADATA.SYNC_CONFIG_POLICIES (
...
-- SHOW POSTGRES INSTANCES only returns instances where you have OPERATE or OWNERSHIP.
-- Run this for each Postgres instance you want the app to manage:
GRANT OPERATE ON POSTGRES INSTANCE <name> TO ROLE SYSADMIN;
...
-- ============================================================
-- Note: Procedures use 'pg_secret_<INSTANCE_ID>' labels to pick the right credentials.
-- When adding more instances via the Admin UI, the app automatically rebuilds
-- procedures with additional secret bindings.
...
SECRETS = ('pg_secret_1' = PGSYNC_DB.METADATA.PG_SECRET, 'pg_secret' = PGSYNC_DB.METADATA.PG_SECRET)
EXECUTE AS CALLER;
